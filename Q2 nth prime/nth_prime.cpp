...
// g++ -o nth_prime.exe nth_prime.cpp PrimeCalculator.cpp -std=c++11
// ./nth_prime.exe 100
...
{
    std::cout << "Prime Calculator" << std::endl;
    ...
    std::cout << "The " << N << "th prime number is : ";
...
bool TestPrimeCalculator(){
    ...
    for (int Idx = 0; Idx < PrimesPositions.size(); Idx++){
        if (Calculator.CalculateNthPrime(PrimesPositions[Idx]) != PrimesNumbers[Idx]){
             std::cout << "Failed test : " << PrimesPositions[Idx] << "  should give " << PrimesNumbers[Idx] << std::endl;
            return false;
        }
    }
...
// Note :
// Le 1000000 e nombre premier est : 15485863
